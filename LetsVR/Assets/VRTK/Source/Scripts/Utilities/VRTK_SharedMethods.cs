...
namespace VRTK
{
	...
	public static class VRTK_SharedMethods
    {
        ...
        public static bool Vector2ShallowCompare(Vector2 vectorA, Vector2 vectorB, int compareFidelity)
        {
            ...
            return (Math.Round(Mathf.Abs(distanceVector.x), Math.Abs(compareFidelity), MidpointRounding.AwayFromZero) < float.Epsilon &&
                    Math.Round(Mathf.Abs(distanceVector.y), Math.Abs(compareFidelity), MidpointRounding.AwayFromZero) < float.Epsilon);
        }
