namespace VRTK
{
    ...
    public class VRTK_VRInputModule : PointerInputModule
    {
        ...
        VRTK_UIGraphicRaycaster[] raycasters;
        ...
        public override void Process()
        {
            for (int i = 0; i < pointers.Count; i++)
            {
                ...
                if (pointer.gameObject.activeInHierarchy && pointer.enabled)
                {
                    ...
                    {
                        raycasters = FindObjectsOfType<VRTK_UIGraphicRaycaster>();
                        results = CheckRaycasts(pointer);

                        if (results.Any(x => pointer.prioritizedGameObjects.Any(y => x.gameObject.name.Contains(y))))
                        {
                            List<RaycastResult> temp = results.Where(x => pointer.prioritizedGameObjects.Contains(x.gameObject.name) || pointer.prioritizedGameObjects.Contains(x.module.name)).ToList();
                            results = results.Where(x => temp.Any(y => y.distance == x.distance)).ToList();
                        }
                    }
        ...
        protected virtual List<RaycastResult> CheckRaycasts(VRTK_UIPointer pointer)
        {
            ...
            //List<List<RaycastResult>> raycasts2 = new List<List<RaycastResult>>();
            
            foreach (var raycaster in raycasters)
            {
                List<RaycastResult> raycasts2 = new List<RaycastResult>();
                raycaster.Raycast(pointer.pointerEventData, raycasts2);
                raycasts.AddRange(raycasts2);
            }
            List<RaycastResult> raycasts3 = new List<RaycastResult>();
            eventSystem.RaycastAll(pointer.pointerEventData, raycasts3);
            raycasts.AddRange(raycasts3);
