...
namespace VRTK
{
    ...
    public class VRTK_Pointer : VRTK_DestinationMarker
    {
        ...
        {
            if (enabled && givenHit.collider.transform != null && (!ControllerRequired() || VRTK_ControllerReference.IsValid(controllerReference)))
            {
                SetHoverSelectionTimer(givenHit.collider);

                DestinationMarkerEventArgs destinationEventArgs = SetDestinationMarkerEvent(givenHit.distance, givenHit.collider.transform, givenHit, givenHit.point, controllerReference, false, GetCursorRotation());
                ...
                StartUseAction(givenHit.collider.transform);
            }
        }
        ...
        public virtual void PointerExit(RaycastHit givenHit)
        {
            ...
            if (givenHit.collider != null && (!ControllerRequired() || VRTK_ControllerReference.IsValid(controllerReference)))
            {
                OnDestinationMarkerExit(SetDestinationMarkerEvent(givenHit.distance, givenHit.collider.transform, givenHit, givenHit.point, controllerReference, false, GetCursorRotation()));
                StopUseAction();
            }
        }
...
#pragma warning restore 0618
        ...
        {
            controllerEvents = (controllerEvents == null ? GetComponentInParent<VRTK_ControllerEvents>() ?? GetComponent<VRTK_ControllerEvents>() : controllerEvents);
        ...
        protected virtual void ExecuteSelectionButtonAction()
        {
            if (EnabledPointerRenderer() && CanSelect() && (IsPointerActive() || wasActivated))
            {
                ...
                if (pointerRendererDestinationHit.collider == null)
                    return;

                AttemptUseOnSet(pointerRendererDestinationHit.collider.transform);
                if (pointerRendererDestinationHit.collider.transform && IsPointerActive() && pointerRenderer.ValidPlayArea() && !PointerActivatesUseAction(pointerInteractableObject) && pointerRenderer.IsValidCollision())
                {
                    ResetHoverSelectionTimer(pointerRendererDestinationHit.collider);
                    ResetSelectionTimer();
                    OnDestinationMarkerSet(SetDestinationMarkerEvent(pointerRendererDestinationHit.distance, pointerRendererDestinationHit.collider.transform, pointerRendererDestinationHit, pointerRendererDestinationHit.point, controllerReference, false, GetCursorRotation()));
                }
            }
        }
        ...
        protected virtual DestinationMarkerEventArgs GetStateEventPayload()
        {
            ...
            if (EnabledPointerRenderer())
            {
                RaycastHit pointerRendererDestinationHit = pointerRenderer.GetDestinationHit();
                if (pointerRendererDestinationHit.collider != null)
                    eventPayload = SetDestinationMarkerEvent(pointerRendererDestinationHit.distance, pointerRendererDestinationHit.collider.transform, pointerRendererDestinationHit, pointerRendererDestinationHit.point, controllerReference, false, GetCursorRotation());
            }
            return eventPayload;
        }
    }
