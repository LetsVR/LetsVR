...
namespace BeardedManStudios.Forge.Networking.Unity.Modules
{
	public class VOIP
	{
		...
		private void Setup(string hostAddress, ushort port)
		{
			...
			{
				if (UseTcp)
					((TCPServer)Socket).Connect(hostAddress, port);
				else
			...
			else
			{
				...
				if (UseTcp)
					((TCPClient)Socket).Connect(hostAddress, port);
				else
		...
		{
			if (UseTcp)
				Socket = new TCPServer(connections);
			else
		...
		private void VOIPWorker()
		{
			while (Socket.IsConnected)
			{
				if (writeFlushTimer >= WRITE_FLUSH_TIME && writeSamples.Count > 0)
				{
					...
					if (Socket is BaseUDP)
						((BaseUDP)Socket).Send(voice);
					else if (Socket is TCPServer)
						((TCPServer)Socket).Send(voice);
					else if (Socket is TCPClient)
						((TCPClient)Socket).Send(voice);
				}
