...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ControllerSyncNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ControllerSyncNetworkObject.Length > 0 && ControllerSyncNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ControllerSyncNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ControllerSyncBehavior>();
						}
					}
			...
			else if (obj is MoleculeSyncObjectNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MoleculeSyncObjectNetworkObject.Length > 0 && MoleculeSyncObjectNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MoleculeSyncObjectNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<MoleculeSyncObjectBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateControllerSync instead, its shorter and easier to type out ;)")]
		public ControllerSyncBehavior InstantiateControllerSyncNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ControllerSyncNetworkObject[index]);
			var netBehavior = go.GetComponent<ControllerSyncBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ControllerSyncBehavior>().networkObject = (ControllerSyncNetworkObject)obj;
		...
		[Obsolete("Use InstantiateMoleculeSyncObject instead, its shorter and easier to type out ;)")]
		public MoleculeSyncObjectBehavior InstantiateMoleculeSyncObjectNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MoleculeSyncObjectNetworkObject[index]);
			var netBehavior = go.GetComponent<MoleculeSyncObjectBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MoleculeSyncObjectBehavior>().networkObject = (MoleculeSyncObjectNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ControllerSync
		/// </summary>
		/// <returns>
		/// A local instance of ControllerSyncBehavior
		/// </returns>
		/// <param name="index">The index of the ControllerSync prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ControllerSyncBehavior InstantiateControllerSync(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (ControllerSyncNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: ControllerSync. Add them at the NetworkManager prefab.");
				return null;
			}
		...
		/// <summary>
		/// Instantiate an instance of MoleculeSyncObject
		/// </summary>
		/// <returns>
		/// A local instance of MoleculeSyncObjectBehavior
		/// </returns>
		/// <param name="index">The index of the MoleculeSyncObject prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public MoleculeSyncObjectBehavior InstantiateMoleculeSyncObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (MoleculeSyncObjectNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: MoleculeSyncObject. Add them at the NetworkManager prefab.");
				return null;
			}
